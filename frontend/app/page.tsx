...
function DonutChart({ data }: { data: Distribution[] }) { return <ResponsiveContainer width="100%" height="100%"><PieChart><Tooltip contentStyle={tooltipStyle} labelStyle={{ color: "#f8fbff", fontWeight: 600 }} itemStyle={{ color: "#edf5ff" }} formatter={(value, name) => [formatNumber(Number(value)), humanize(String(name))]} /><Pie data={data} dataKey="value" nameKey="name" innerRadius={65} outerRadius={95} paddingAngle={3}>{data.map((item, index) => <Cell key={item.name} fill={chartColors[index % chartColors.length]} />)}</Pie></PieChart></ResponsiveContainer>; }
